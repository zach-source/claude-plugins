...
- `agent` - Execute in specialized sub-agent context
- `context` - Execution context (use `fork` for isolated execution)
...
**Type:** String
**Required:** No
**Default:** None (uses current agent)

**Purpose:** Execute the command in a specialized sub-agent context
...
1. **Specialized analysis:** Command needs domain expertise
   ```yaml
   ---
   description: Deep security audit of codebase
   agent: security-auditor
   ---
   ```

2. **Complex workflows:** Command benefits from agent tools
   ```yaml
   ---
...
3. **Language-specific tasks:** Command needs language expertise
   ```yaml
   ---
...
Match those in the Task tool (e.g., `code-reviewer`, `debugger`, `typescript-pro`, `architect-reviewer`, `security-auditor`, `golang-pro`, `python-pro`, etc.)

**Best practices:**
- Only use when command genuinely benefits from specialized agent
- Match agent to the primary task domain
- Consider combining with `context: fork` for isolation

### context

**Type:** String
**Required:** No
**Default:** None (inline execution)
**Values:** `fork`
...
- Command runs in an isolated sub-agent context
- Forked agent has full conversation history up to invocation
- Results return to main conversation when complete
- Useful for complex, multi-step tasks that benefit from isolation

**When to use:**

1. **Complex analysis:** Commands doing extensive exploration
   ```yaml
   ---
...
2. **Long-running tasks:** Commands that may take many steps
   ```yaml
   ---
...
3. **Combined with agent:** Specialized agent in isolated context
   ```yaml
   ---
...
**Best practices:**
- Use `fork` when command does extensive multi-step work
- Combine with `agent` for specialized isolated execution
- Results summarize back to main conversation
...
Complex task in isolated context:
...
- [ ] agent is valid Task tool agent type if specified
- [ ] context is `fork` if specified (only valid value)
...
5. **Use agents wisely:** Only specify agent when specialized expertise needed
6. **Fork for complexity:** Use `context: fork` for multi-step isolated tasks
7. **Manual-only sparingly:** Only use disable-model-invocation when necessary
8. **Clear descriptions:** Make commands discoverable in `/help`
9. **Test thoroughly:** Verify frontmatter works as expected
